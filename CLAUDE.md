...
The data-fetching layer is built (env var, Zod-validated client, TanStack Query hooks). The **location-suggestions dropdown is wired into `SearchBar`**; the **current-weather lookup is not yet wired anywhere** — the Search/Clear buttons are still inert, `WeatherResult`/`NotFoundBanner` are still static, and `App.tsx` still renders hardcoded sample data. Wiring the Search button to a real query (and its error states to `NotFoundBanner`) is future work. What exists today:
...
- **Client**: `src/features/weather/api/openWeatherClient.ts` — `fetchCurrentWeather(city, country)` and `fetchLocationSuggestions(city, country)`, both throwing a typed `OpenWeatherApiError` (`status` + a `reason` union — `'not-found'` is the specific signal a future task should key `NotFoundBanner` off of, distinct from other failure reasons). Also exports `shouldRetryOpenWeatherQuery`, a shared TanStack Query `retry` function that skips retrying deterministic 4xx failures (a 5xx *is* retried — that's deliberate, don't "fix" it into always skipping retries).
- **Hooks**: `src/features/weather/hooks/useCurrentWeatherQuery.ts` and `useLocationSuggestionsQuery.ts` — thin `useQuery` wrappers (reads, not mutations — this is cacheable GET data, which is what makes "search again from history" cheap once history exists). `useCurrentWeatherQuery` takes `{ city, country } | null` and stays disabled on `null`, **not yet called from anywhere**. `useLocationSuggestionsQuery` stays disabled until the city text reaches `MIN_CITY_QUERY_LENGTH` (exported from that file) and does **not** debounce internally by design — `SearchBar` is the one debouncing (see below).
...
- **`SearchBar`'s suggestions dropdown** (`src/features/weather/components/SearchBar.tsx`): City/Country are controlled inputs. Typing debounces (350ms, via `lodash/debounce` — use the same subpath-import style for any future debounce need, not a different library or the full `lodash` barrel import) into `useLocationSuggestionsQuery`, rendered as a combobox-pattern dropdown (`role="combobox"`/`listbox`/`option`, arrow-key + Enter navigation) covering loading/results/empty/error states with the shared theme tokens. Selecting a suggestion fills both inputs and stops there — it does not trigger a weather lookup.
...
- **Color tokens, not repeated `dark:` pairs**: `src/index.css` defines the palette as CSS custom properties rather than leaving every component to hand-roll a color plus its `dark:` counterpart. Two layers:
  ...
  - **Reusable component classes** (`@layer components`, same file): `.glass-panel`, `.icon-button`, `.field-label`, `.eyebrow-label` capture visual patterns already repeated across ≥2 components. Reach for an existing one before copy-pasting a component's utility string, and add a new one here (built with `@apply` against the tokens above) once a third occurrence of the same pattern shows up — don't invent a class for something used exactly once.
